(async function () {
  ...
  function render() {
    ...
    const visible = providers.filter((provider) => {
      ...
      const matchesText = [provider.id, provider.name, provider.state, provider.activeDomain, (provider.domains || []).join(" "), (provider.formats || []).join(" "), (provider.languages || []).join(" ")].join(" ").toLowerCase().includes(q);
    ...
    grid.innerHTML = visible.map((provider) => {
      ...
      return `<article class="provider-card ${cls}">
        ...
        <div class="provider-domains">${domains.slice(0, 3).map((domain) => `<span>${Madrador.esc(domain)}</span>`).join("")}</div>
