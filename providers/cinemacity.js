...
// Promise-based only for Nuvio sandbox compatibility.

var MAIN_URL = "https://cinemacity.cc";
var TMDB_API_KEY = "1865f43a0549ca50d341dd9ab8b29f49";

// Do not publish personal cookies. Keep this empty in the public repo.
...
function getHeaders(extra) {
  ...
  if (CINEMACITY_COOKIE) headers.Cookie = CINEMACITY_COOKIE;
...
function collectTvStreams(fileData, title, seasonNum, episodeNum) {
  ...
  for (var i = 0; i < fileData.length; i++) {
    ...
    if (seasonTitle.indexOf("Season " + seasonNum) !== -1 || seasonTitle.indexOf("S" + seasonNum) !== -1) {
  ...
  for (var j = 0; j < seasonObject.folder.length; j++) {
    ...
    if (episodeTitle.indexOf("Episode " + episodeNum) !== -1 || episodeTitle.indexOf("E" + episodeNum) !== -1) {
    ...
    .then(function (mediaInfo) {
      ...
      return findMediaUrl(title).then(function (mediaUrl) {
        ...
        return fetchText(mediaUrl).then(function (pageHtml) {
          ...
          if (!fileData) return [];
          if (mediaType === "movie") return collectMovieStreams(fileData, title);
