...
function renderTestPlayerPage() {
  return "<!doctype html>" +
    ...
    "<title>Test lecture</title><style>:root{color-scheme:dark;--bg:#101214;--panel:#181b1f;--soft:#20242a;--line:#30363d;--text:#f6f8fb;--muted:#a9b2bf;--accent:#8b5cf6;--accent2:#14b8a6;--bad:#ef4444}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Segoe UI,Arial,sans-serif;line-height:1.5}main{width:min(1160px,calc(100% - 32px));margin:0 auto;padding:30px 0 46px}.top{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin-bottom:18px}h1{font-size:38px;line-height:1;margin:0 0 8px}h2{margin:0 0 12px;font-size:20px}.muted{color:var(--muted)}.links{display:flex;gap:10px;flex-wrap:wrap}.link{color:#d8c7ff;text-decoration:none;border:1px solid var(--line);border-radius:8px;padding:9px 12px;background:var(--panel)}video{width:100%;max-height:58vh;background:#000;border-radius:8px;border:1px solid var(--line)}button,input,select{min-height:42px;border-radius:8px;border:1px solid var(--line);font:inherit}button{padding:0 14px;background:var(--accent);color:white;font-weight:700;cursor:pointer}button.secondary{background:var(--soft)}button.ghost{background:#080a0c;color:var(--text)}input,select{background:#080a0c;color:var(--text);padding:0 12px}.search{display:grid;grid-template-columns:1fr 138px auto;gap:10px;margin:16px 0}.layout{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:16px}.panel{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:14px;min-height:170px}.results{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.card{display:grid;grid-template-columns:54px 1fr;gap:10px;width:100%;text-align:left;background:#080a0c;border:1px solid var(--line);border-radius:8px;color:var(--text);padding:8px}.card img{width:54px;height:78px;object-fit:cover;border-radius:6px;background:#252a31}.card strong,.stream strong{display:block}.card small,.stream small{display:block;color:var(--muted);font-weight:400}.stream{width:100%;text-align:left;background:#080a0c;border:1px solid var(--line);border-radius:8px;color:var(--text);font-weight:600;padding:10px;margin:0 0 8px}.stream.active{border-color:var(--accent2);box-shadow:0 0 0 1px var(--accent2)}.bad{color:var(--bad)}.now{display:flex;align-items:center;justify-content:space-between;gap:12px;background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:10px 12px;margin-top:10px}.tools{display:flex;gap:8px;flex-wrap:wrap}pre{white-space:pre-wrap;background:#080a0c;border:1px solid var(--line);border-radius:8px;padding:12px;color:#cbd5e1;overflow:auto;max-height:180px;margin-top:14px}@media(max-width:860px){.top,.layout,.search{grid-template-columns:1fr;display:grid}.results{grid-template-columns:1fr}.links,.tools{display:grid}.link,button{width:100%}}</style></head>" +
    "<body><main><div class=\"top\"><div><h1>Test lecture</h1><p class=\"muted\">Cherche un film ou une serie, choisis un resultat, puis teste les streams directement ici.</p></div><nav class=\"links\"><a class=\"link\" href=\"/\">Accueil</a><a class=\"link\" href=\"/status\">Statut</a><a class=\"link\" href=\"/manifest.json\">Manifest</a></nav></div><video id=\"video\" controls playsinline></video><div class=\"now\"><div><strong id=\"nowTitle\">Aucun stream lance</strong><div id=\"nowUrl\" class=\"muted\">Choisis un stream pour commencer.</div></div><div class=\"tools\"><button id=\"copy\" class=\"secondary\">Copier l'URL</button><button id=\"open\" class=\"ghost\">Ouvrir</button></div></div>" +
    ...
    "<div class=\"layout\"><section class=\"panel\"><h2>Resultats</h2><div id=\"results\" class=\"muted\">Aucune recherche lancee.</div></section><section class=\"panel\"><h2>Streams</h2><div id=\"streams\" class=\"muted\">Choisis un resultat.</div></section></div><pre id=\"log\">Pret.</pre></main>" +
    "<script src=\"https://cdn.jsdelivr.net/npm/hls.js@1\"></script><script>" +
    "const log=document.getElementById('log'),video=document.getElementById('video'),q=document.getElementById('query'),type=document.getElementById('type'),searchBtn=document.getElementById('search'),results=document.getElementById('results'),streamsBox=document.getElementById('streams'),nowTitle=document.getElementById('nowTitle'),nowUrl=document.getElementById('nowUrl'),copyBtn=document.getElementById('copy'),openBtn=document.getElementById('open');let hls=null,currentUrl='';" +
    "function write(x){log.textContent+='\\n'+x}function setLog(x){log.textContent=x}function esc(x){return String(x||'').replace(/[&<>\\\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\\"':'&quot;'}[c]))}function formatUrl(u){const s=String(u||'').replace(location.origin,'');const m=s.match(/^\\/proxy\\/[^/]+\\/(stream\\.[a-z0-9]+)$/i);return m?'/proxy/.../'+m[1]:s}" +
    "async function search(){const query=q.value.trim();if(!query)return;setLog('Recherche: '+query);results.textContent='Recherche...';streamsBox.textContent='Choisis un resultat.';const data=await fetch('/search.json?type='+encodeURIComponent(type.value)+'&q='+encodeURIComponent(query)).then(r=>r.json());results.className='results';results.innerHTML=data.results.map(r=>'<button class=\"card\" data-id=\"'+r.id+'\" data-type=\"'+r.type+'\"><img src=\"'+esc(r.poster||'')+'\" alt=\"\"><span><strong>'+esc(r.title)+'</strong><small>'+esc(r.year||'Annee inconnue')+' - TMDB '+r.id+'</small></span></button>').join('')||'<span class=\"muted\">Aucun resultat</span>';results.querySelectorAll('button').forEach(b=>b.onclick=()=>loadStreams(b.dataset.type,b.dataset.id,b.innerText.trim()));write('Resultats: '+data.results.length)}" +
    "async function loadStreams(mediaType,id,label){setLog('Streams pour '+label+'...');streamsBox.textContent='Chargement...';const endpoint='/stream/'+mediaType+'/'+id+'.json';const data=await fetch(endpoint).then(r=>r.json());streamsBox.innerHTML=data.streams.map((s,i)=>{const kind=s.url.includes('.mp4')?'MP4':s.url.includes('.m3u8')?'HLS':'LINK';return '<button class=\"stream\" data-i=\"'+i+'\"><strong>'+esc(s.name)+' - '+kind+'</strong><small>'+esc(s.title||s.description||'')+'<br>'+esc(formatUrl(s.url))+'</small></button>'}).join('')||'<span class=\"muted\">Aucun stream</span>';streamsBox.querySelectorAll('button').forEach(b=>b.onclick=()=>play(data.streams[Number(b.dataset.i)],b));write('Streams: '+data.streams.length);if(data.streams[0]) play(data.streams[0],streamsBox.querySelector('button'))}" +
    "async function play(s,button){if(!s)return;streamsBox.querySelectorAll('.active').forEach(x=>x.classList.remove('active'));if(button)button.classList.add('active');currentUrl=s.url;nowTitle.textContent=s.name+' - '+(s.url.includes('.mp4')?'MP4':s.url.includes('.m3u8')?'HLS':'Lien');nowUrl.textContent=formatUrl(s.url);write('Lecture: '+s.name+' - '+(s.title||s.description||''));write(s.url);if(hls){hls.destroy();hls=null}video.removeAttribute('src');video.load();if(s.url.includes('.m3u8')){if(window.Hls&&Hls.isSupported()){hls=new Hls({enableWorker:true,lowLatencyMode:false});hls.loadSource(s.url);hls.attachMedia(video);hls.on(Hls.Events.ERROR,(e,d)=>write('HLS error: '+JSON.stringify({type:d.type,details:d.details,fatal:d.fatal})));}else if(video.canPlayType('application/vnd.apple.mpegurl')){video.src=s.url}else{write('HLS non supporte dans ce navigateur');return}}else{video.src=s.url}await video.play().catch(e=>write('Lecture bloquee: '+e.message))}" +
