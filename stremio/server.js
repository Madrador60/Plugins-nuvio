...
<main>
  ...
  <section class="hero"><div><span class="chip">Catalogue beta</span><h1>Films et series</h1><p class="lead">Cherche un titre, ouvre sa fiche, puis lance directement la lecture ou Stremio.</p><div class="search"><input id="query" placeholder="Rechercher un titre..." value=""><select id="type"><option value="movie">Film</option><option value="series">Serie</option></select><button id="go">Rechercher</button></div></div></section>
  <div id="searchResults"></div>
  <div id="rows" class="loading">Chargement du catalogue...</div>
</main>
<nav class="bottomNav"><a href="/">Accueil</a><a href="/test-player">Lecteur</a><a href="/providers">Providers</a></nav>
<div id="modal" class="modal"><button id="close" class="close">X</button><div id="sheet" class="sheet"></div></div>
<script>
const rows=document.getElementById('rows'),searchResults=document.getElementById('searchResults'),query=document.getElementById('query'),type=document.getElementById('type'),go=document.getElementById('go'),modal=document.getElementById('modal'),sheet=document.getElementById('sheet'),closeBtn=document.getElementById('close');
...
go.onclick=()=>searchCatalog().catch(e=>{searchResults.innerHTML='<div class="empty">Erreur recherche: '+esc(e.message||e)+'</div>'});query.addEventListener('keydown',e=>{if(e.key==='Enter')go.click()});closeBtn.onclick=()=>modal.className='modal';modal.onclick=e=>{if(e.target===modal)modal.className='modal'};load();
</script>
</body>
</html>`;
}
