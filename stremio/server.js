...
function esc(x){return String(x||'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
function openSearch(t,kind,id,year){location.href='/test-player?q='+encodeURIComponent(t)+'&type='+encodeURIComponent(kind)+'&id='+encodeURIComponent(id||'')+'&year='+encodeURIComponent(year||'')+'&autoload=1'}
