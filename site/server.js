...
async function testProvider(card){const id=card.dataset.id,btn=card.querySelector('.testBtn'),out=card.querySelector('.testResult');if(card.dataset.testable!=='1')return null;btn.classList.add('loading');btn.textContent='Test...';out.textContent='En cours...';out.className='testResult';try{const data=await fetch('/diagnostics.json?providers='+encodeURIComponent(id)).then(r=>r.json());const r=data.results&&data.results[0];const status=r?r.status:'ERROR';out.textContent=status+' · '+(r?r.streams:0)+' source(s) · '+(r?r.timeMs:0)+'ms';out.className='testResult '+(status==='OK'?'ok':status==='ZERO'?'warn':'bad');return r}catch(e){out.textContent='Erreur: '+(e.message||e);out.className='testResult bad';return null}finally{btn.classList.remove('loading');btn.textContent='Tester'}}
